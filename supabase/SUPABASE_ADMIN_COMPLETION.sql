-- ═══════════════════════════════════════════════════════════════════════
-- The QR Gate — ADMIN COMPLETION (consolidated, paste-ready reference).
--
-- This file is byte-for-byte the body of supabase/migrations/0006_admin_completion.sql.
-- Apply EITHER the migration OR this file (not both is required — both are the
-- same idempotent script). Run it in the Supabase SQL Editor AFTER 0000–0005.
--
-- Execution order:  0000 → 0001 → 0002 → 0003 → 0004 → 0005 → (this) 0006
--
-- After applying, /admin's new pages (Analytics, Reports, System Health,
-- Security, Support), the QR/subscription/audit/search RPCs, admin notes,
-- and the last-super-admin guard become live. Until applied, those pages
-- degrade to an honest "migration required" state (never fake data).
--
-- Safety: no credentials, no real user UUIDs, no Stripe/Vercel secrets.
-- Every privileged function is SECURITY DEFINER with search_path='' and
-- re-checks the caller's admin role; admin tables have SELECT-only RLS
-- (writes only via these definer functions / the service role).
-- ═══════════════════════════════════════════════════════════════════════
