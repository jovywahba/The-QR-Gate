-- ───────────────────────────────────────────────────────────────
-- The QR Gate — Part 5: auth profiles, $10 Pro subscription, QR
-- ownership quota, and real scan analytics.
--
-- RLS-first (hard gate §10). Idempotent: safe to run once on top of
-- 0000_init + 0001_qr_codes. Billing rows are written ONLY by the
-- service role (Stripe webhook); scan events ONLY by the service role
-- (scan routes). End users never insert either.
-- ───────────────────────────────────────────────────────────────
